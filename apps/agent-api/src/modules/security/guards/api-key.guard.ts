    ...
    try {
      ...
      if (requiredTypes && requiredTypes.length > 0) {
        if (!requiredTypes.includes(keyInfo.type)) {
          throw new ForbiddenException(`API key type ${keyInfo.type} is not allowed for this endpoint`);
        }
      }
      ...
      if (!rateLimitPassed) {
        ...
        response.setHeader('X-RateLimit-Reset', Date.now() + (customLimit?.windowSeconds || 60) * 1000);
      ...
      await this.securityService.logSecurityEvent({
        eventType: error.message.includes('rate limit') ? 'RATE_LIMIT_EXCEEDED' as any : 'UNAUTHORIZED_ACCESS' as any,
        ipAddress,
        userAgent,
        apiKey,
