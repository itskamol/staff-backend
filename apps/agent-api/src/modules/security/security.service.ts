...
export class SecurityService {
  ...
  async validateApiKey(apiKey: string, ipAddress: string, userAgent: string): Promise<ApiKeyInfo> {
  ...
  async checkRateLimit(identifier: string, ipAddress: string, userAgent: string, customLimit?: RateLimitConfigDto): Promise<boolean> {
  ...
  async listApiKeys(): Promise<Array<Omit<ApiKeyInfo, 'id'> & { keyId: string; maskedKey: string }>> {
  ...
  async addIpToWhitelist(ipDto: IpWhitelistDto): Promise<void> {
    ...
    this.logger.log(`IP added to whitelist: ${ipDto.ipAddress} - ${ipDto.description || 'No description'}`);
  }
  ...
  async logSecurityEvent(eventDto: SecurityEventDto): Promise<void> {
    ...
    if (event.severity && event.severity >= 7) {
      this.logger.warn(`Security Event: ${event.eventType} from ${event.ipAddress}`, event.details);
    } else {
  ...
  async getSecurityEvents(limit: number = 100, eventType?: SecurityEventType): Promise<SecurityEventDto[]> {
