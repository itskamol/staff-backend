...
describe('SecurityService', () => {
  ...
  describe('security events', () => {
    ...
    it('should filter events by type', async () => {
      ...
      const rateLimitEvents = await service.getSecurityEvents(10, SecurityEventType.RATE_LIMIT_EXCEEDED);
      const invalidKeyEvents = await service.getSecurityEvents(10, SecurityEventType.INVALID_API_KEY);

      expect(rateLimitEvents.every(e => e.eventType === SecurityEventType.RATE_LIMIT_EXCEEDED)).toBe(true);
      expect(invalidKeyEvents.every(e => e.eventType === SecurityEventType.INVALID_API_KEY)).toBe(true);
    });
  });
