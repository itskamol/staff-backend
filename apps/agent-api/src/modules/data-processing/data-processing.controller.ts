...
export class DataProcessingController {
  ...
  @ApiOperation({ 
    ...
    description: 'Submits a single data processing job to the queue for asynchronous processing'
  })
  @ApiResponse({ 
    status: 202, 
  ...
  @ApiOperation({ 
    ...
    description: 'Submits multiple data processing jobs in a single request for better throughput'
  })
  @ApiResponse({ 
    status: 202, 
  ...
  @ApiResponse({ 
    ...
    schema: {
      ...
      properties: {
        batchId: { type: 'string' },
        jobs: { type: 'array', items: { $ref: '#/components/schemas/ProcessingResultDto' } },
        summary: {
          type: 'object',
          properties: {
            total: { type: 'number' },
  ...
  async getPerformanceMetrics() {
    ...
    return {
      ...
      errorRates: {
        overall: queueStatus.totalJobs > 0 ? (queueStatus.failedJobs / queueStatus.totalJobs) * 100 : 0,
