...
export class HIKVisionService {
  ...
  async processBatchActions(batchDto: HIKVisionBatchActionDto): Promise<HIKVisionActionResponse[]> {
    ...
    this.logger.log(`Batch processing completed: ${successCount}/${batchDto.actions.length} actions processed successfully`);
  ...
  async processEvent(eventDto: HIKVisionEventDto): Promise<HIKVisionEventResponse> {
    ...
    try {
      this.logger.debug(`Processing HIKVision event: ${eventDto.eventType} from device ${eventDto.deviceIp}`);
      ...
      switch (eventDto.eventType) {
        ...
        default:
          this.logger.debug(`Event type ${eventDto.eventType} processed without special handling`);
      }
  ...
  async processBatchEvents(batchDto: HIKVisionBatchEventDto): Promise<HIKVisionEventResponse[]> {
    ...
    const successCount = responses.filter(r => r.processed).length;
    this.logger.log(`Batch event processing completed: ${successCount}/${batchDto.events.length} events processed successfully`);

    return responses;
  }

  async processDeviceStatus(statusDto: HIKVisionDeviceStatusDto): Promise<HIKVisionDeviceStatusResponse> {
    ...
    try {
      ...
      if (statusDto.status === HIKVisionDeviceStatus.OFFLINE || statusDto.status === HIKVisionDeviceStatus.ERROR) {
  ...
  async processBatchDeviceStatus(batchDto: HIKVisionBatchDeviceStatusDto): Promise<HIKVisionDeviceStatusResponse[]> {
    ...
    this.logger.log(`Batch device status processing completed: ${successCount}/${batchDto.devices.length} devices processed successfully`);
      ...
      visitor = await this.prisma.visitor.findFirst({
        where: {
          OR: [
            { 
              AND: [
                { firstName: { contains: personInfo.name.split(' ')[0], mode: 'insensitive' } },
                { lastName: { contains: personInfo.name.split(' ')[1] || '', mode: 'insensitive' } },
              ],
            },
          ],
        },
  ...
  private async generateSecurityAlert(actionDto: HIKVisionActionDto, device: any) {
    this.logger.warn(`Security Alert: Access denied at device ${device.name} (${actionDto.deviceIp})`);
  ...
  private async generateDeviceAlert(statusDto: HIKVisionDeviceStatusDto, device: any) {
    this.logger.warn(`Device Alert: ${device?.name || statusDto.deviceIp} status is ${statusDto.status}`);
