...
export class AgentService {
  ...
  private async findOrCreateUsersOnComputers(computerUserId: number, computerId: number): Promise<number> {
    ...
    if (!usersOnComputers) {
      ...
      this.logger.log(`Created new users on computers relation: ${computerUserId} - ${computerId}`);
    }
  ...
  async processActiveWindows(activeWindowDto: ActiveWindowDto, apiKey: string) {
    ...
    try {
      ...
      const usersOnComputersId = await this.findOrCreateUsersOnComputers(computerUserId, computerId);
      ...
      this.logger.log(`Processed ${activeWindows.length} active window records for ${computerUid}/${userSid}`);
    } catch (error) {
  ...
  async processVisitedSites(visitedSiteDto: VisitedSiteDto, apiKey: string) {
    ...
    try {
      ...
      this.logger.log(`Processed ${visitedSites.length} visited site records for ${computerUid}/${userSid}`);
    } catch (error) {
  ...
  async processScreenshots(screenshotDto: ScreenshotDto, apiKey: string) {
    ...
    try {
      ...
      this.logger.log(`Processed ${screenshots.length} screenshot records for ${computerUid}/${userSid}`);
    } catch (error) {
  ...
  async processUserSessions(userSessionDto: UserSessionDto, apiKey: string) {
    ...
    try {
      ...
      this.logger.log(`Processed ${userSessions.length} user session records for ${computerUid}/${userSid}`);
    } catch (error) {
