...
export class QueueService implements OnModuleDestroy {
    ...
    async add<T>(type: string, data: T, options: QueueOptions = {}): Promise<string> {
    ...
    private async processJob(job: QueueJob): Promise<void> {
        ...
        try {
            ...
            if (job.attempts >= job.maxAttempts) {
                ...
                this.logger.warn(
                    `Job failed, will retry: ${job.id} (attempt ${job.attempts}/${job.maxAttempts})`
                );
            }
    ...
    private async getWaitingJobs(): Promise<QueueJob[]> {
        return Array.from(this.jobs.values()).filter(
            job => this.getJobStatus(job) === 'waiting' || this.getJobStatus(job) === 'delayed'
        );
    }
