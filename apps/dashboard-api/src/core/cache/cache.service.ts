...
export class CacheService implements OnModuleDestroy {
  ...
  async getSessionCache<T>(sessionId: string, key: string, options?: CacheOptions): Promise<T | null> {
    return this.get<T>(`session:${sessionId}:${key}`, options);
  }

  async setSessionCache<T>(sessionId: string, key: string, value: T, options?: CacheOptions): Promise<void> {
