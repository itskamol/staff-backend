...
import { QueryBuilderUtil, PaginationDto, EncryptionUtil } from '@staff-control-system/shared/utils';
...
export class VisitorService {
  ...
  async findOne(id: number, user: any) {
    const visitor = await this.prisma.visitor.findUnique({
      ...
      select: {
        ...
        onetimeCodes: user.role === Role.GUARD ? false : {
          select: {
            id: true,
            codeType: true,
            code: true,
  ...
  async deactivateCode(codeId: number, user: any) {
    ...
    // Check access permissions
    if (user.role === Role.HR && onetimeCode.visitor.creator.organizationId !== user.organizationId) {
