...
export class EmployeeService {
    ...
    const [employees, totalRecords] = await Promise.all([
      this.prisma.employee.findMany({
        ...
        select: {
          ...
          policy: user.role === Role.GUARD ? false : {
            select: {
              id: true,
              title: true,
            },
          },
          _count: user.role === Role.GUARD ? false : {
            select: {
              computerUsers: true,
              credentials: true,
              actions: true,
  ...
  async findOne(id: number, user: any) {
    const employee = await this.prisma.employee.findUnique({
      ...
      select: {
        ...
        computerUsers: user.role === Role.GUARD ? false : {
          select: {
            id: true,
            sid: true,
            name: true,
        ...
        credentials: user.role === Role.GUARD ? false : {
          select: {
            id: true,
            code: true,
            type: true,
    ...
    if (user.role === Role.DEPARTMENT_LEAD && !user.departmentIds?.includes(employee.department.id)) {
      throw new ForbiddenException('Access denied to this employee');
    }
  ...
  async remove(id: number, user: any) {
    ...
    if (employeeWithDeps?.computerUsers.length || employeeWithDeps?.credentials.length || employeeWithDeps?.actions.length) {
      // Soft delete
  ...
  async getActivityReport(id: number, paginationDto: PaginationDto, user: any) {
    ...
    if (computerUsers.length === 0) {
      return QueryBuilderUtil.buildResponse([], 0, paginationDto.page || 1, paginationDto.limit || 10);
    }
