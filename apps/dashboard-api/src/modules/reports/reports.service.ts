...
export class ReportsService {
  ...
  async generateReport(generateReportDto: GenerateReportDto, user: any) {
    const { reportType, timeRange, startDate, endDate, departmentIds, employeeIds, exportFormat } = generateReportDto;
  ...
  async generateAttendanceReport(dto: AttendanceReportDto, user: any): Promise<AttendanceReportData[]> {
  ...
  async generateProductivityReport(dto: ProductivityReportDto, user: any): Promise<ProductivityReportData[]> {
    ...
    for (const employee of employees) {
      ...
      for (const computerUser of employee.computerUsers) {
        for (const userOnComputer of computerUser.usersOnComputers) {
          ...
          for (const window of userOnComputer.activeWindows) {
            ...
            applicationUsage.set(appName, (applicationUsage.get(appName) || 0) + window.activeTime);
          }
      ...
      const productivityPercentage = totalActiveTime > 0 ? (productiveTime / totalActiveTime) * 100 : 0;
  ...
  async generateDeviceUsageReport(dto: DeviceUsageReportDto, user: any): Promise<DeviceUsageReportData[]> {
    ...
    for (const device of devices) {
      ...
      const daysDiff = Math.ceil(
        (new Date(endDate || new Date()).getTime() - new Date(startDate || '2024-01-01').getTime()) 
        / (1000 * 60 * 60 * 24)
      );
  ...
  async generateVisitorActivityReport(dateRange: any, user: any): Promise<VisitorActivityReportData[]> {
    // Build where clause based on user role
    const whereClause: any = {
      createdAt: {
