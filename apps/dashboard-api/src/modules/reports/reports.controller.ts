...
export class ReportsController {
  ...
  async getAttendanceReport(
    ...
    const report = await this.reportsService.generateAttendanceReport(attendanceReportDto, user);
  ...
  async getProductivityReport(
    ...
    const report = await this.reportsService.generateProductivityReport(productivityReportDto, user);
  ...
  @ApiResponse({ status: 200, description: 'Device usage report retrieved successfully' })
  async getDeviceUsageReport(
    @Query() deviceUsageReportDto: DeviceUsageReportDto,
    @CurrentUser() user: any,
  ): Promise<ApiResponseDto> {
    const report = await this.reportsService.generateDeviceUsageReport(deviceUsageReportDto, user);
  ...
  @ApiResponse({ status: 200, description: 'Visitor activity report retrieved successfully' })
  ...
  async getDashboardSummary(
    ...
    const summary = {
      ...
      averageAttendance: attendanceReport.length > 0 
      ...
      totalEntriesToday: deviceUsageReport.reduce((sum, device) => sum + device.totalEntries, 0),
